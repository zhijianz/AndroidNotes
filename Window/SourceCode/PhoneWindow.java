...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
     ...
     * @author zhijianz
     * 
     * 这个地方已经写出了window的view体系结构，mDecor作为最顶层的容器包含mContentRoot，
     * mContentRoot存在的目的是为了针对不同的window-style实现不同的布局主题，然后包含
     * mContentPrent，这个就是我们常说的android.R.id.content，在这个容器里面可以增加多个
     * 期望的显示内容, 最常见的使用方式是Activity中的应用常见，一个是在onCreate中调用
     * setContentView设置Activity中需要显示的内容，另外一个是在后期业务扩展的时候可以
     * 使用addContentView增加新的内容，同样的逻辑也可以应用在mDecor中 
     */
     ...
     * @author zhijianz
     *
     * setContentView 和 addContentView 这一块的逻辑，除了简单的对mContentParent中
     * 的内容进行替换或增加之外，还有另外的两块逻辑，一个是考虑内容更新的时候转场
     * 动画，然而这个属性需要API21，另外一个也是最重要的一点，就是初始化mDecor和
     * mContentParent两个容器，参见installDecor
     * 
     */
    @Override
    public void setContentView(int layoutResID) {
        ...
        // zhijianz 这个callback应该是activity实现并且在activity初始化的时候赋值给window
    ...
    private void installDecor() {
        ...
        if (mContentParent == null) {
            ...
            // zhijianz 这里的DecorContentParent只是一个接口，按照网上的说法，应该是一个ActionBarOverlayLayout
            final DecorContentParent decorContentParent = (DecorContentParent) mDecor.findViewById(
                    R.id.decor_content_parent);

            // zhijianz 暂时不考虑decorContentParent存在的情况

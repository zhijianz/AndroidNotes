...
public abstract class Window {
    ...
    /**
     * 其实到了这里会有一个比较疑惑的地方，对于window外观和行为的控制会
     * 存在两套不同体系：LayoutParams&Feature
     */
    ...
    /**
     * 在这个位置定义的callbak，在后面Activity实现的时候会通过开放
     * 一些接口来实现这些callba对应的功能
     */
    /**
    ...
    /**
     * 在这个地方就可以发现，其实window相关的属性值还依然存在LayoutParams中
     * Feature只用来设置其是否具备这个属性而已
     * @param wp [description]
     */
    ...
    /**
     * 1. 必须要在setContentView之前调用
     * 2. 不调用该函数不会原有任何属性
     * 3. 属性一旦设定之后不能够关掉该属性，理由？
     */
    /**
...
/**
 * 整个Window的源代码大部分接口都围绕着LayoutParams和Feature进行，这两个参数同时控制着window的外观和行为
 * 另外一个比较重要的部分是window定义的callback，响应各种改动，最后回调应该会回到Activity中
 */